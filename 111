(function() {
    ...
    const tgLink = "https://t.me/ProfStroyShopBot?start=site";

    // Стили (исправлены: центровка, размеры, адаптивность)
    ...
    style.innerHTML = `
        ...
        #tg-v { width: 72px; height: 72px; border-radius: 50%; object-fit: cover; position: absolute; z-index: 2; }
        ...
        #tg-icon-bubble { position: absolute; right: 83px; top: -5px; width: 20px; z-index: 20; }
        ...
        #tg-fb span { margin-left: 8px; transition: opacity 0.3s; }
        #tg-fb.circle span { display: none; }
        @media (hover: hover) { 
            #tg-fb.circle:hover { width: 185px; padding: 0 12px !important; } 
            #tg-fb.circle:hover img { margin-right: 8px !important; } 
            #tg-fb.circle:hover span { display: inline; } 
        }
    `;
    ...
    container.innerHTML = `
        <div id="tg-widget">
            <div id="tg-bubble">
                ${!isMobile ? '<video id="tg-v" muted autoplay loop playsinline preload="auto"><source src="https://videotourl.com/videos/1776719073148-fbb2c3ca-2969-4293-adeb-e95391355c1e.mp4"></video>' : '<div style="width:72px;height:72px;border-radius:50%;background:#229ED9;display:flex;align-items:center;justify-content:center;position:absolute;z-index:2;"><img src="https://upload.wikimedia.org/wikipedia/commons/5/5a/Telegram_black_logo.svg" style="width:32px;filter:invert(1)"></div>'}
                <svg id="ring" viewBox="0 0 100 100"><circle cx="50" cy="50" r="46" fill="none" stroke="rgba(255,255,255,0.2)" stroke-width="3"></circle><circle id="tg-progress" cx="50" cy="50" r="46" fill="none" stroke="#fff" stroke-width="3" stroke-dasharray="289" stroke-dashoffset="289"></circle></svg>
                <div id="online"></div>
                <div id="panel"><div id="text-field"></div></div>
                <div id="tg-icon-bubble"><img src="https://upload.wikimedia.org/wikipedia/commons/8/83/Telegram_2019_Logo.svg" style="width:100%"></div>
                <div id="close-btn">×</div>
            </div>
        </div>
        <a id="tg-fb" href="${tgLink}" target="_blank">
            <img src="https://upload.wikimedia.org/wikipedia/commons/5/5a/Telegram_black_logo.svg">
    ...
    const typeText = () => {
        const msgs = ["Подскажем материал под задачу","Выберем затирку без ошибок","⚡ Пишите нам в Telegram"];
        let idx = 0;
        ...
        const run = () => {
            ...
            const timer = setInterval(() => {
                ...
                if (i > msgs[idx].length) { clearInterval(timer); idx++; setTimeout(run, 1800); }
            }, 40);
        };
        run();
    };
    ...
    const close = () => {
        ...
        setTimeout(() => {
            ...
            setTimeout(() => fb.classList.add('show', 'circle'), 100);
        }, 500);
    };
    ...
    if (localStorage.getItem('tg_closed')) {
        ...
        const onScroll = () => { if (window.scrollY > 200) { showWidget(); window.removeEventListener('scroll', onScroll); } };
    ...
    setInterval(() => {
        const v = document.getElementById('tg-v');
        if (v && v.duration) document.getElementById('tg-progress').style.strokeDashoffset = 289 - (289 * (v.currentTime / v.duration));
    }, 50);
})();
